...
|--------|------|-------------------|-------------|-------------|
| kubeflow | org | 48 | ~101,939 | 2026-06-17 |
| bmorphism | user | 104 | ~453 | 2026-06-15 |
| plurigrid | org | 100 | ~157 | 2026-06-17 |
| AustinCStone | user (social graph) | 40 | ~319 | 2026-04-01 |
| TeglonLabs | org | 5 | ~14 | 2026-06-08 |
| zubyul | user | 49 | ~31 | 2026-04-24 |
| migalkin | user (social graph) | 19 | ~805 | 2026-05-28 |
| wasita | user (social graph) | 11 | ~10 | 2026-06-15 |
| M1shaaa | user (social graph) | 8 | 0 | 2026-02-04 |
| kristinezheng | user (social graph) | 5 | 0 | 2026-06-07 |
| DJedamski | user (social graph) | 6 | ~16 | 2023-04-21 |

**Total:** ~395 repos across 11 sources

### Notable Repos (most recently active)

**plurigrid** (active 2026-06-17):
- `gorj` — forj + Rama topology nREPL routing + GF(3) gay trit coloring — 633 open issues
- `place` — TeX repo pushed 2026-06-15
- `asi` — everything is topological chemputer! — 26 stars

**kubeflow** (active 2026-06-17):
- `kubeflow` — Machine Learning Toolkit for Kubernetes — 15,726 stars
- `pipelines` — ML Pipelines for Kubeflow — 4,154 stars
- `mcp-apache-spark-history-server` — MCP Server for Spark — 177 stars (recently added)

**bmorphism** (active 2026-06-15):
- `Gay.jl` — Wide-gamut color sampling, SPI pattern — 187 open issues
- `satreadout` — Machine-checked saturating perceptual readout in Lean
- `ocaml-mcp-sdk` — OCaml SDK for MCP (Jane Street oxcaml_effect) — 61 stars

**TeglonLabs** (active 2026-06-08):
- `jank-crane` — crane-jank converged-IR hub with GF3 convergence maps

**zubyul** (active 2026-04-24):
- `voice-observatory` — Passive macOS TUI for voice-download pathways
- `tilelang-kernels` — TileLang GPU kernels for GF(3) trit classification, targeting GB10 Blackwell

**migalkin** (social graph, active 2026-05-28):
- `NodePiece` — Knowledge Graph representations (ICLR'22) — 144 stars
- `StarE` — Hyper-Relational KG (EMNLP 2020) — 89 stars

---

## JOB 2: Hamming Swarm Snapshot (Aptos Mainnet)

**Timestamp:** 2026-06-17 UTC  
**Wallets Probed:** 28 (alice, bob, A-Z)

### Wallet Balances

All 28 Hamming swarm wallets returned **0 APT** on Aptos mainnet.
...
All 5 multisig pairs healthy (2-of-2 via `0x1::multisig_account::num_signatures_required`):
...
**Status: UNAVAILABLE** — `testnet.mnx.fi` requires Vercel deployment authentication.
The endpoint returned a Vercel protection challenge. No market data was extractable from the SPA.
...
| Sweep # | trit | Hex | Name |
|---------|------|-----|------|
| 1 (this sweep) | +1 | #b8bb26 | PLUS |
| 2 (next) | -1 | #cc241d | MINUS |
| 3 | 0 | #d3869b | ERGODIC |
...
├── world_increments    — 1 sweep record (GF3 chain)
├── repo_snapshots      — 1,134 rows (11 sources, ~395 unique repos)
├── aptos_snapshots     — 28 rows (all 0 APT balance)
