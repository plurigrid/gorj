...
| Org | Total Repos | Top Repo | Stars |
|-----|------------|----------|-------|
| plurigrid | 101 | plurigrid/asi | 26 |
| kubeflow | 48 | kubeflow/kubeflow | 15,739 |
| TeglonLabs | 5 | TeglonLabs/mathpix-gem | 2 |

**plurigrid highlights (most recently pushed):**
- `plurigrid/gorj` — Clojure — 729 open issues — pushed 2026-06-21 — forj + Rama topology nREPL routing + GF(3) gay trit coloring
- `plurigrid/place` — TeX — 9 open issues — pushed 2026-06-20
- `plurigrid/asi` — HTML — ★26 — pushed 2026-06-10 — everything is topological chemputer!
- `plurigrid/eirobri` — Clojure — 29 open issues — pushed 2026-06-03 — EiRoBri replay world

**kubeflow highlights (most recently pushed):**
- `kubeflow/katib` — Python — ★1,684 — Automated Machine Learning on Kubernetes
- `kubeflow/pipelines` — Python — ★4,156 — Machine Learning Pipelines for Kubeflow
- `kubeflow/trainer` — Go — ★2,118 — Distributed AI Model Training and LLM Fine-Tuning
- `kubeflow/spark-operator` — Python — ★3,127 — Kubernetes operator for Apache Spark
...
| User | Total Repos | Most Recent Push |
|------|-------------|-----------------|
| bmorphism | 105 | bmorphism/Gay.jl (2026-06-21) |
| zubyul | 49 | zubyul/voice-observatory (2026-04-24) |

**bmorphism highlights:**
- `bmorphism/Gay.jl` — Julia — ★2 — 187 open issues — Wide-gamut color sampling with splittable determinism — pushed 2026-06-21
- `bmorphism/satreadout` — HTML — Machine-checked saturating non-Riemannian perceptual readout — pushed 2026-06-20
- `bmorphism/ocaml-mcp-sdk` — OCaml — ★61 — OCaml SDK for Model Context Protocol

**zubyul highlights:**
- `zubyul/voice-observatory` — Python — Passive macOS TUI observing voice-download pathways
- `zubyul/gay-world` — Python — ★1 — Goblin world builder: each goblin is a world

### Zubyul Social Graph

| User | Total Repos | Notes |
|------|-------------|-------|
| migalkin | 19 | KG researcher; StarE (★89), NBFNet_mlx (★10) |
| DJedamski | 6 | R/data science; older repos (2014–2023) |
| wasita | 11 | Svelte/Python; active 2026 |
| kristinezheng | 5 | Cognitive science, HackMIT, MIT |
| M1shaaa | 8 | TypeScript/Python; Lookit developmental studies |
| AustinCStone | 40 | Python; bitmind/EpsteinSearch work |

**Social graph highlights:**
- `migalkin/StarE` — Python — ★89 — EMNLP 2020: Message Passing for Hyper-Relational KGs
- `migalkin/NBFNet_mlx` — Python — ★10 — Neural Bellman-Ford networks in MLX for Apple Silicon
- `wasita/wasita.github.io` — Svelte — active personal site, pushed 2026-06-15

### GF(3) Color Chain

Increment IDs colored by `id % 3`:
- **trit=0 (ERGODIC) `#d3869b`** — entries 3, 6, 9, 12, ...
- **trit=1 (PLUS) `#b8bb26`** — entries 1, 4, 7, 10, ...
- **trit=-1 (MINUS) `#cc241d`** — entries 2, 5, 8, 11, ...

Total world_increments in DB: **71** (48 this sweep + 23 accumulated from prior sweeps)
...
All 28 Hamming addresses (alice, bob, A–Z) queried against Aptos mainnet `fullnode.mainnet.aptoslabs.com`.
...
**Note:** All accounts returned 0.0 APT. The `CoinStore<0x1::aptos_coin::AptosCoin>` resource was not found for any address, indicating these wallets have not been funded on mainnet (accounts exist but have no initialized coin store).
...
**All 5 multisig contracts healthy** — each requires 2-of-N signatures, all responsive.

### MNX Markets

`https://testnet.mnx.fi` — **UNAVAILABLE**

Vercel deployment protection active on testnet.mnx.fi. Authentication required (Vercel SSO / trusted-source OIDC / protection bypass token). No market data extractable without credentials. mnx_snapshots table has 0 rows this sweep.
...
*Generated by world-increment-sweep + hamming-swarm-snapshot autonomous agent · 2026-06-21*
