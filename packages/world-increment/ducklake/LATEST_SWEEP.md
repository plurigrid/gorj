...
| Total Repo Snapshots | 391 (646 unique full_names across sources) |
...
GF(3) cycle repeats across all 391 repo increments.
...
- `M1shaaa/M1shaaa` — pushed **2026-06-25** (today, active)
- `wasita/wasita.github.io` (Svelte personal site) — pushed **2026-06-25** (today, active)
...
- plurigrid & bmorphism: 100 repos each snapshotted (full metadata in DB)
- kubeflow: 48 repos (flagship ML/Kubernetes platform org)
...
**Finding:** All 28 accounts exist on mainnet (confirmed via `/v1/accounts/{addr}`, have sequence numbers) but the legacy `CoinStore` resource was not found. Accounts likely use the newer `FungibleStore` mechanism or hold zero APT. All balances recorded as `0.0 APT`.
...
**Status: Unavailable** — `testnet.mnx.fi` returns HTTP 401 requiring Vercel visitor password authentication. API paths `/api/markets` and `/api/v1/markets` both blocked. No market data extractable without credentials.
...
```
world_increments  — 391 GF3-tagged repo increment events
repo_snapshots    — full repo metadata (language, stars, forks, pushed_at, description)
aptos_snapshots   — 28 wallet balances (all 0.0 APT via CoinStore; accounts confirmed active)
multisig_probes   — 5 multisig contracts (all 2-of-N, all healthy)
mnx_snapshots     — 1 row: status=unavailable (auth required)
```

---

*Generated by world-increment-sweep + hamming-swarm-snapshot agent — 2026-06-25*
