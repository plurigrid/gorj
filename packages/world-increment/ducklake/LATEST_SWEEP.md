...
- **Increment ID:** 13
- **GF(3):** trit=1 (+1), color=#b8bb26, name=**PLUS**
...
| Source | Result |
|--------|--------|
| org: plurigrid | 1 repo via MCP (gorj) |
| org: kubeflow | rate-limited (no gh CLI / unauthenticated) |
| org: TeglonLabs | rate-limited |
| user: bmorphism | rate-limited |
| user: zubyul | rate-limited |
| social graph: migalkin, DJedamski, wasita, kristinezheng, M1shaaa, AustinCStone | rate-limited |

**Note:** `gh` CLI not available; unauthenticated GitHub REST API is IP-rate-limited. MCP tools scoped to `plurigrid/gorj` only. Orgs/users from prior sweeps (2026-04-12) remain in the cumulative repo_snapshots.

### plurigrid/gorj — Latest State (MCP)
| Field | Value |
|-------|-------|
| Language | Clojure |
| Description | MCP server + hooks that give AI coding agents a Clojure REPL |
| Latest commit | ebf263f — 2026-04-14 (sync world.duckdb sweep state) |
| Active branches | 90+ world-increment sweep branches (2026-04-27 → 2026-05-06) |
| Most recent branch | world-increment/sweep-2026-05-06-1510 |
...
| Sources Previously Covered | 3 orgs + 8 users (from 2026-04-12 sweep) |

---

## JOB 2: Hamming Swarm Snapshot

### Aptos Wallet Balances (Mainnet, queried 2026-05-06)
API: `GET /v1/accounts/{addr}/resource/0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>`

All 28 addresses returned **0.0 APT** — CoinStore resource not initialized on mainnet for these addresses.
...
All 5 multisig contracts live and healthy: **2-of-2 signatures required**.

### MNX Markets (testnet.mnx.fi)
- **Status: UNAVAILABLE** — Next.js SPA, no public REST API
- `/api/markets` and `/api/v1/markets` return HTML
- Recorded as `MNX_UNAVAILABLE` in mnx_snapshots

---

## GF(3) Color Chain (recent)

| ID | Date | Source | GF3 Trit | Color | Name |
|----|------|--------|----------|-------|------|
| 10 | 2026-04-10 | M1shaaa (user) | +1 | `#b8bb26` | PLUS |
| 11 | 2026-04-10 | AustinCStone (user) | -1 | `#cc241d` | MINUS |
| 12 | 2026-04-12 | bmorphism (org) | 0 | `#d3869b` | ERGODIC |
| **13** | **2026-05-06** | **plurigrid (org)** | **+1** | **`#b8bb26`** | **PLUS** |
