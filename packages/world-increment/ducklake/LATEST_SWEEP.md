...
**Sweep timestamp:** 2026-04-26T06:32 UTC  
...
|----------|------|------:|----------|
| kubeflow | (portfolio) | ~101,663 total | Python/Go |
...
341 increment records written to `world_increments` with GF(3) color chain.
...
Queried via Aptos fullnode `coin::balance` view function. Balances in APT (octas ÷ 1e8).
...
**Total swarm APT:** ~19.71 APT  
**bob** holds the dominant position at 12.657 APT (64.2% of swarm).
...
**Status: Unavailable** — `/api/markets` and `/api/tickers` return 404 SPA pages. The backend API (`api.testnet.mnx.fi`) requires authenticated WebSocket. No public REST market data endpoint found. `mnx_snapshots` table is empty.
...
*Generated by world-increment-sweep + hamming-swarm-snapshot agent on 2026-04-26.*
