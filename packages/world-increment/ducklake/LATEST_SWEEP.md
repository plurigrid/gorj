# World-Increment Sweep — 2026-05-05
...
### GF(3) Color Chain — New Increments (2026-05-05)
...
|--------|------|-------|-------|-------|
| kubeflow | org | 47 | ~33,908 | ~13,367 |
| bmorphism | user | 100 | ~170 | ~33 |
| AustinCStone | user | 40 | ~108 | ~37 |
| plurigrid | org | 100 | ~49 | ~26 |
| zubyul | user | 49 | ~14 | ~1 |
| migalkin | user | 19 | ~274 | ~48 |
| TeglonLabs | org | 4 | 2 | 2 |
| DJedamski | user | 6 | ~3 | ~2 |
| wasita | user | 10 | ~4 | ~1 |
| kristinezheng | user | 6 | 0 | 0 |
| M1shaaa | user | 8 | 0 | 0 |

### Notable New Repos (2026-05-05)

- **kubeflow/kubeflow**: flagship ML platform for Kubernetes (~15k stars)
- **migalkin/NodePiece**: 143★ scalable KG embeddings (ICLR'22)
- **migalkin/StarE**: 89★ hyper-relational KG message passing (EMNLP 2020)
- **AustinCStone/TextGAN**: 92★ TensorFlow text GAN
- **AustinCStone/StereoVisionMRF**: 11★ MRF stereo depth inference
- **TeglonLabs/mathpix-gem**: Ruby math OCR → LaTeX (2 stars, 11 open issues)
- **TeglonLabs/coin-flip-mcp**: MCP server for random.org coin flips
- **wasita/magic-garden**: Discord game bot, active April 2026
- **wasita/wasita.github.io**: Svelte/SvelteKit personal site, active April 2026
- **bmorphism/boxxy** (top pushed for bmorphism)

### Zubyul Social Graph Nodes Swept
...
**Snapshot timestamp:** 2026-05-05T13:18Z
**Chain:** Aptos Mainnet (`fullnode.mainnet.aptoslabs.com`)
**Wallets probed:** 28 (alice, bob, A–Z)

### APT Balance Summary

All 28 wallets returned **0.0 APT** from the `0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>` resource at this snapshot time. Accounts appear unfunded or hold no native APT.
...
All 5 multisig contracts resolved via `0x1::multisig_account::num_signatures_required`.
All healthy — **2-of-N** threshold confirmed on-chain.
...
**Status: SPA only — no machine-readable API available**

`testnet.mnx.fi` serves a Next.js single-page app. Probed `/api/markets` and `/api/v1/markets` — both return the SPA HTML shell without embedded JSON. No market data extractable without browser execution. `mnx_snapshots` table remains empty this sweep.

---

## DuckDB Tables (Cumulative State)

```
world_increments   — 34 rows
repo_snapshots     — 1264 rows
...
*Previous sweep: 2026-04-12 (12 increments, 471 repos)*
*This sweep: 2026-05-05 (11 new increments, ~389 new repo rows)*
*Next sweep: increment ids continue from max(id)+1 following GF(3) color chain*
