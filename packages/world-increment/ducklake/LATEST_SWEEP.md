...
**Sweep ID:** world-increment/sweep-2026-06-04  
...
- `world_increments` — 381 rows (GF3 color-chained increment log)
- `repo_snapshots` — 381 rows (org/user, full_name, language, stars, forks, open_issues, pushed_at)
...
### Notable Active Repos (pushed 2026-06-04)

| Repo | Stars | Language | Description |
|------|-------|----------|-------------|
| plurigrid/gorj | 0 | Clojure | forj + Rama topology nREPL routing + GF(3) gay trit coloring |
| bmorphism/Gay.jl | 1 | Julia | Wide-gamut color sampling with splittable determinism (189 open issues) |
| M1shaaa/M1shaaa | 0 | — | Config files for GitHub profile |
| kubeflow/trainer | 2110 | Go | Distributed AI Model Training and LLM Fine-Tuning on Kubernetes |
| kubeflow/sdk | 120 | Python | Universal Python SDK for AI workloads on Kubernetes |
...
28 addresses (alice, bob, A-Z) queried via Aptos fullnode REST API.
**All balances = 0.0 APT** — CoinStore resources not initialized on mainnet.
...
testnet.mnx.fi is a Next.js single-page application. API paths probed:
`/api/markets`, `/api/v1/markets`, `/api/tickers` — all returned no structured JSON market data.
