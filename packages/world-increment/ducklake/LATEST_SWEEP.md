...
| kubeflow/examples | 1,462 | Jupyter Notebook |
...
### Most Recently Active (plurigrid org)
| Repo | Pushed At | Stars | Description |
|------|-----------|-------|-------------|
| plurigrid/gorj | 2026-06-07 | 0 | forj + Rama topology nREPL routing + GF(3) gay trit coloring |
| plurigrid/eirobri | 2026-06-03 | 0 | EiRoBri replay world |
| plurigrid/place | 2026-06-04 | 1 | |
| plurigrid/nash-portal | 2026-05-19 | 2 | NASH token TUI in the browser |
| plurigrid/asi | 2026-04-26 | 25 | everything is topological chemputer! |
| plurigrid/asi-skills | 2026-04-26 | 3 | 69 skills with Galois Hole Type accessibility |

### bmorphism Most Active
| Repo | Pushed At | Stars | Language |
|------|-----------|-------|----------|
| bmorphism/flox-mcp-bb | 2026-06-05 | 0 | Clojure |
| bmorphism/babashka-mcp-server | 2026-06-05 | 19 | JavaScript |
| bmorphism/world | 2026-06-02 | 0 | Python |
| bmorphism/Gay.jl | 2026-05-15 | 1 | Julia |
| bmorphism/ocaml-mcp-sdk | 2026-05-08 | 61 | OCaml |

### GF(3) Color Chain Distribution (191 increments)
...
GF(3) cycle: `PLUS → MINUS → ERGODIC → PLUS → MINUS → ERGODIC → ...` (63 full cycles + 2 remainder)
...
28 addresses (alice, bob, A–Z) probed via  
`fullnode.mainnet.aptoslabs.com/v1/accounts/{addr}/resource/0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>`

**Result:** All 28 addresses returned **0.0 APT** — CoinStore resource absent for all.  
Accounts either have no APT holdings or do not exist on Aptos mainnet.
...
All 5 contracts respond to `0x1::multisig_account::num_signatures_required` — **all healthy**.
...
**UNAVAILABLE** — testnet.mnx.fi requires Vercel authentication at every endpoint  
(`/`, `/api/markets`, `/api/v1/markets`). The SPA returns a Vercel auth challenge.  
No market data accessible without Vercel CLI or MCP credentials.
...
world_increments  — 191 rows  (GF(3) color-chained repo sweep events)
repo_snapshots    — 191 rows  (org/user, language, stars, forks, issues, pushed_at)
aptos_snapshots   — 28 rows   (alice, bob, A–Z wallet balances @ 0.0 APT each)
multisig_probes   — 5 rows    (A-B, A-G, Y-Z, S-T, V-W; all 2-sig threshold, healthy)
mnx_snapshots     — 1 row     (UNAVAILABLE — Vercel auth required)
```
...
- **plurigrid/gorj** (this repo): most recently pushed 2026-06-07, 408 open issues — hot active dev
- **plurigrid/asi**: 25 stars — topological chemputer, recently active
- **bmorphism**: MCP-centric ecosystem spanning OCaml, Clojure, JavaScript, Zig, Julia, Rust, Go
- **zubyul** social graph: Rust TUIs (nash-tui), Gay.jl color work, plurigrid site infra
- **migalkin** social graph: knowledge graph ML (NodePiece 144★, StarE 89★), MLX work
- **Hamming swarm A–Z**: all mainnet balances are 0.0 APT — staging/test addresses unfunded
- **All 5 multisig contracts**: 2-sig threshold confirmed live and queryable on Aptos mainnet
