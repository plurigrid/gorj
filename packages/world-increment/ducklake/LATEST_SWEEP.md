...
GF(3) rule: `id%3==1 → PLUS | id%3==2 → MINUS | id%3==0 → ERGODIC`

### Repo Counts by Source

| Source | Type | Repos | Total Stars |
|--------|------|-------|-------------|
| plurigrid | org | 100 | 73 |
| bmorphism | user | 100 | 248 |
| zubyul | user | 49 | 14 |
| kubeflow | org | 48 | 34,054 |
| migalkin | user (zubyul graph) | 5 | 275 |
| AustinCStone | user (zubyul graph) | 3 | 103 |
| wasita | user (zubyul graph) | 4 | 3 |
| TeglonLabs | org | 4 | 2 |
...
### Notable Recent Pushes (2026-05-15)
...
- `kristinezheng/kristinezheng.github.io` — GitHub Pages site
...
**Status:** All 28 addresses returned `resource_not_found` for `CoinStore<AptosCoin>` — addresses exist on-chain but APT CoinStore is uninitialized (standard for fresh/unused accounts).
...
**Status:** Unavailable — SPA (Next.js) returns HTML shell for all API paths. No REST endpoint exposes JSON market data; rendering is client-side only. `mnx_snapshots` table is empty.
...
|-------|------|-------------|
| `world_increments` | 320 | GF(3)-tagged increment events per repo |
| `repo_snapshots` | 320 | GitHub repo metadata (stars, forks, language, pushed_at) |
| `aptos_snapshots` | 28 | Hamming swarm wallet balances (all 0.0 APT) |
| `multisig_probes` | 5 | Multisig health checks (all 2-sig, healthy) |
