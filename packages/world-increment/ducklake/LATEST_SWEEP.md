...
**Sweep timestamp:** 2026-05-02 (UTC)
...
|--------|------|-------------------|-------------|
| kubeflow | org | 104 | 96,418 |
| bmorphism | user | 210 | 409 |
| plurigrid | org | 210 | 113 |
| AustinCStone | user (social) | 91 | 322 |
| migalkin | user (social) | 65 | 821 |
| wasita | user (social) | 63 | 9 |
| zubyul | user | 56 | 30 |
| TeglonLabs | org | 110 | 14 |
| kristinezheng | user (social) | 38 | 0 |
| M1shaaa | user (social) | 34 | 0 |
| DJedamski | user (social) | 25 | 17 |
...
All 28 addresses (alice, bob, A–Z) queried against Aptos mainnet ledger v5086327714.
All returned `resource_not_found` for `0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>`.
These accounts have not initialized an APT CoinStore — **balance = 0.0 APT for all worlds**.
...
**All 5 multisig accounts are 2-of-2 and healthy.**

### MNX Markets (testnet.mnx.fi)

`https://testnet.mnx.fi` serves a Next.js SPA. No REST API endpoints (`/api/markets`,
`/api/v1/markets`, etc.) return structured data — only HTML is served.
**Status: unavailable / SPA-only. No market data captured (0 rows in mnx_snapshots).**
...
aptos_snapshots       28
multisig_probes        5
mnx_snapshots          0  (SPA — no accessible API)
```
...
*Generated by world-increment-sweep + hamming-swarm-snapshot agent on 2026-05-02*
