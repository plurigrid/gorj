...
**plurigrid** — GF(3)-native org; `gorj` (175 open issues, Clojure REPL+MCP), `asi` (21★, topological chemputer), `zig-syrup` (OCapN Syrup in Zig)

**kubeflow** — ML infra powerhouse; `kubeflow/kubeflow` (15,629★), `spark-operator` (3,125★), `pipelines` (4,140★), `katib` AutoML (1,682★)

**bmorphism** — GF(3)/Gay.jl core contributor; `Gay.jl` (188 open issues, wide-gamut SPI coloring), `ocaml-mcp-sdk` (61★ OCaml MCP), `anti-bullshit-mcp-server` (23★)

**zubyul** — plurigrid social graph; `gay-world`, `tilelang-kernels` (GPU kernels for GF(3)), `voice-observatory`

**migalkin** — KG researcher; `NodePiece` (144★ compositional KG embeddings), `StarE` (89★ hyper-relational KGs)

### GF(3) Trit Distribution (world_increments)

| Name | Color | Count |
|------|-------|-------|
| ERGODIC (trit=0) | #d3869b | 64 |
| PLUS (trit=1) | #b8bb26 | 65 |
| MINUS (trit=-1) | #cc241d | 65 |

### Events

- bmorphism/events: rate-limited (unauthenticated GitHub API)
- zubyul/events: rate-limited (unauthenticated GitHub API)
- MCP tools restricted to `plurigrid/gorj`; social graph queried via GitHub search API
...
All 28 Hamming-swarm addresses (alice, bob, A-Z) were probed against Aptos mainnet ledger version ~5,254,835,573. Every account returned `resource_not_found` for `0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>`, indicating **unfunded accounts** (APT balance = 0.0 for all).
...
All 5 probed multisig contracts respond correctly via `0x1::multisig_account::num_signatures_required`:
...
All multisig contracts healthy: 2-of-N threshold confirmed on all pairs.

### MNX Markets (testnet.mnx.fi)

Status: **UNAVAILABLE** — site serves a Next.js SPA with no public REST API endpoints. Probed `/api/markets`, `/api/v1/markets`, `/api/tickers`, `/api/v1/tickers`, `/api/pairs`, `/api/stats` — all returned HTML. `mnx_snapshots` table is empty.
...
|-------|------|-------------|
| world_increments | 194 | GF(3) color-chained repo events |
| repo_snapshots | 194 | org/user/repo metadata |
| aptos_snapshots | 28 | Hamming swarm wallet balances |
| multisig_probes | 5 | Aptos multisig health |
| mnx_snapshots | 0 | unavailable (SPA) |
