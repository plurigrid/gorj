...
**Sweep timestamp:** 2026-04-13T15:50:00Z  
...
| user | wasita | 31 |
| user | M1shaaa | 16 |
| user | DJedamski | 0 (private/empty) |
| user | kristinezheng | 0 (private/empty) |
| user | AustinCStone | 0 (private/empty) |

**Total repo snapshots:** 401  
**Total world_increments rows:** 461 (401 repos + 60 events)

### GF(3) Color Chain Distribution

| GF(3) Trit | Color Name | Hex | Count |
|-----------|------------|-----|-------|
| 0 | ERGODIC | #d3869b | 153 |
| 1 | PLUS | #b8bb26 | 154 |
| -1 | MINUS | #cc241d | 154 |
...
|-------|-----------|-------|
| bmorphism | PushEvent | 26 |
| bmorphism | PullRequestEvent | 1 |
| bmorphism | WatchEvent | 1 |
| bmorphism | CreateEvent | 1 |
| bmorphism | ForkEvent | 1 |
| zubyul | CreateEvent | 11 |
| zubyul | PullRequestEvent | 10 |
| zubyul | PushEvent | 8 |
| zubyul | ReleaseEvent | 1 |
...
> All addresses returned 0.0 APT: CoinStore resource not initialized on mainnet for these accounts.
...
`testnet.mnx.fi` is a Next.js single-page application. REST probes to all common exchange API paths returned HTTP 404:
...
- `/api/stats` - 404
- `/v1/ticker` - 404
- `/exchange/api/markets` - 404

Market data is hydrated client-side. Recorded as `unavailable` in `mnx_snapshots`.
