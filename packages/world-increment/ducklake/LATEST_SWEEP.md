...
|--------|------|-------|-------------|
| kubeflow | org | 48 | 34,179 |
| social_graph | users (migalkin/DJedamski/wasita/kristinezheng/M1shaaa/AustinCStone) | 31 | 395 |
...
### Most Active (Pushed 2026-06-07)

- `plurigrid/gorj` (Clojure, 412 open issues — most active repo in sweep)
- `bmorphism/Gay.jl` (Julia, 189 open issues)

### Social Graph Highlights (Zubyul connections)

- **migalkin** — Knowledge graph researcher; `NodePiece` (144★), `StarE` (89★), `NBFNet_mlx` (10★)
- **DJedamski** — Data scientist; Kaggle / Coursera projects (R, Jupyter)
- **wasita** — Web developer; active Svelte/SvelteKit projects (`wasita.github.io` updated 2026-06-01)
- **kristinezheng** — MIT cognitive science; `lookit-jenga`, HackMIT 2021
- **M1shaaa** — Yale; Lookit studies, TypeScript
- **AustinCStone** — ML/CV; `TextGAN` (92★), `StereoVisionMRF` (11★), `SpectralClustering` (3★)

### GF(3) Distribution (332 repo-events)
...
> Ledger version at query time: ~5,612,747,140 | Epoch: 16,084 | Block height: 814,486,430
...
**Bob is the largest holder** (62.2% of swarm total)  
**F, L, J cluster** at ~1.9 APT each — likely recently funded  
**G, H, I** near dust threshold (~0.001 APT)
...
All 5 multisigs operational — all require 2-of-N signatures and report healthy state.

### MNX Markets (testnet.mnx.fi)

**Status: UNAVAILABLE** — `testnet.mnx.fi` is behind Vercel deployment protection requiring a visitor password or OIDC token. No market data could be extracted. The `mnx_snapshots` table is empty for this sweep.
...
├── world_increments   (332 rows) — GF(3) event chain
├── repo_snapshots     (332 rows) — GitHub repo metadata
├── aptos_snapshots    (28 rows)  — Aptos mainnet balances
├── multisig_probes    (5 rows)   — Multisig health checks
└── mnx_snapshots      (0 rows)   — MNX markets (auth-blocked)
```

---

*Generated by world-increment-sweep + hamming-swarm-snapshot agent — 2026-06-07*  
*GF(3) color chain: ERGODIC(111) PLUS(111) MINUS(110)*
