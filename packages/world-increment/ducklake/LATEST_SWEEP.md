...
## JOB 2 NEW — Hamming Swarm Snapshot (2026-06-02)

### Aptos Mainnet Wallet Balances (alice + A–Z, 28 addresses)

All addresses queried via `0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>` on Aptos mainnet fullnode.  
**Result: All 28 balances = 0.0 APT** — accounts either unregistered or unfunded on mainnet at time of snapshot.
...
- `https://testnet.mnx.fi` → HTTP 200, Next.js SPA ("The AI Exchange")
- `https://testnet.mnx.fi/api/markets` → **404** (no server-side REST API)
- Market data requires JS execution; not extractable via curl/fetch
- **Status: UNAVAILABLE** via REST; 0 rows in mnx_snapshots

### New DuckDB State (2026-06-02)
...
## Previous Sweep (2026-04-12) — Archived Below
