...
**Total repo snapshots: 391 across 11 sources**
...
| bmorphism (top) | varies | mixed | active |
...
**Total world_increment entries: 414**
...
All 28 monitored addresses (alice, bob, A-Z) returned **0.0 APT**.  
...
All 5 probed multisig contracts responded: **2-of-N threshold**, all live on Aptos mainnet.
...
`testnet.mnx.fi` is a Next.js SPA with no exposed public REST API.  
Probed: `/api/markets`, `/api/v1/markets`, `/api/tickers` — all returned HTML shell.  
...
| Table | Rows | Notes |
|-------|------|-------|
| world_increments | 414 | GF(3)-tagged event log |
| repo_snapshots | 1335 | 391 unique repos (forked repos appear in multiple sources) |
| aptos_snapshots | 28 | alice, bob, A-Z; all 0.0 APT |
| multisig_probes | 5 | all 2-of-N, all healthy |
