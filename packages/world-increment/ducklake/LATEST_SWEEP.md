...
Queried via `https://fullnode.mainnet.aptoslabs.com/v1` with 1s delay between calls.
...
All 28 addresses returned 0 APT — `CoinStore<AptosCoin>` resource not found (accounts unfunded on mainnet).

### Multisig Contract Probes (5 pairs)
All probed via `0x1::multisig_account::num_signatures_required` POST to view endpoint.
...
All 5 multisig contracts live on Aptos mainnet with 2-of-N signing threshold.

### MNX Markets (testnet.mnx.fi)
**Status: SPA — JSON API unavailable**  
`testnet.mnx.fi` is a Next.js SPA. No `/api/markets` or `/api/v1/markets` endpoints serve JSON. Market data hydrates client-side only. Recorded as `SPA-unavailable` in `mnx_snapshots`.

---

## JOB 1: GitHub Social Graph Sweep (Updated 2026-05-14)
