...
- **Aptos ledger version:** 5,662,538,615
...
| Source | Type | Repos in DB | Total Available |
|--------|------|-------------|-----------------|
| plurigrid | org | 11 | 101 |
| kubeflow | org | 10 | 48 |
| TeglonLabs | org | 5 | 5 |
| bmorphism | user | 10 | 103 |
| zubyul | user | 5 | 49 |
| migalkin | user (social) | 5 | 19 |
| wasita | user (social) | 3 | 11 |
| DJedamski | user (social) | 2 | 6 |
| kristinezheng | user (social) | 2 | 5 |
| M1shaaa | user (social) | 2 | 8 |
| AustinCStone | user (social) | 3 | 40 |
| **TOTAL** | | **57** | **395** |

**Total repo_snapshots:** 57  
**Total world_increments:** 57  
...
### Most Active (open issues as activity proxy)

| Repo | Open Issues | Last Pushed |
|------|-------------|-------------|
| kubeflow/pipelines | 494 | 2026-06-09 |
| plurigrid/gorj | 474 | 2026-06-10 |
| kubeflow/trainer | 129 | 2026-06-10 |
| kubeflow/katib | 119 | 2026-06-05 |
| kubeflow/mpi-operator | 103 | 2026-06-02 |
| bmorphism/Gay.jl | 189 | 2026-05-15 |

### zubyul Social Graph (full scan: 49 repos)

Active in 2026 Q1/Q2:
- `voice-observatory` — passive macOS TUI observing voice-download pathways; companion to bmorphism/say-mcp-server voice-tree decomposition
- `gay-world` — Goblin world builder with MLX task decomposition → composable persistent worlds
- `big-bad-plurigrid-quiz` — 27 flashcards from bmorphism/plurigrid/zubyul/monaduck1069 activity + Emacs drill
- `tilelang-kernels` — GPU kernels for SplitMix64 color generation, GF(3) trit classification, targeting NVIDIA GB10 Blackwell
...
All 28 addresses queried against Aptos mainnet fullnode at ledger version 5,662,538,615.  
**Result:** All returned `resource_not_found` — no `0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>` resource initialized for any address. These are unregistered accounts.
...
All 5 contracts responded to `POST /v1/view` with `0x1::multisig_account::num_signatures_required`.
...
All pairs are 2-of-N multisig. Network fully responsive.

### MNX Markets (testnet.mnx.fi)

**Status: UNAVAILABLE**  
`testnet.mnx.fi` returns HTTP 401 — Vercel Deployment Protection requiring visitor password or OIDC trusted-source token. No market data extractable. `mnx_snapshots` table is empty this sweep.
...
```
world_increments  57 rows   (GF3-colored repo push events)
repo_snapshots    57 rows   (full repo metadata)
aptos_snapshots   28 rows   (alice, bob, A-Z — all 0.0 APT unregistered)
